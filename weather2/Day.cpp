...
void Day::addSliceOfData(SliceOfData* data) {
	...
	case INSIDE:
		this->insideDataHas = true;
		inside.push_back(data);
		break;
	case OUTSIDE:
		this->outsideDataHas = true;
	...
	*/
	...
	switch (sortType) {
		...
		case MAXDIFFTEMP:
			...
			break;
		case DOOROPENSHENANIGANS:
			liv = this->totalDoorOpenTime.getTotalTimeInSeconds() < compare->getDoorOpenSexonds();
	}
...
void Day::calculateDoor() {
	...
	*/
	...
	// This is completely arbitrary but we lack information and
	// we're not warmth or air or humidity specialists.
	// So it doesn't matter.
	float limit = 0.0001f;

	// A bool for the main loop.
	bool done = false;

	// To not have a gigantic statement of bools inside an if statement.
	bool inValid;
	bool outValid;

	// denominator to make the slope.
	float denominator;
	// result = inslope/difference.
	float result;

	// Tells if door is open right now or closed.
	bool doorOpened = false;

	// To check if we are still finding the suitable time index, for a loop.
	bool findingSuitableTimeIndex = true;

	// The last recorded time for door opening.
	int lastTime;

	// Total amount of time open.
	int timeOpen = 0;
	...
	while (!done) {
		...
		findingSuitableTimeIndex = true;
		while (findingSuitableTimeIndex && (inTrailLow < inside.size() - 1)) {
			if (inside[inTrailLow + 1]->secs() < timeLow)
				inTrailLow++;
			else
				findingSuitableTimeIndex = false;
		}

		// CALCULATE THE HIGHEST EXISTING TIME IN THE TIME WINDOW 
		findingSuitableTimeIndex = true;
		while (findingSuitableTimeIndex && (inTrailHigh < inside.size() - 1)) {
		...
		*/
		...
		// CALCULATE THE LOWEST EXISTING TIME IN THE TIME WINDOW
		findingSuitableTimeIndex = true;
		while (findingSuitableTimeIndex && (outTrailLow < outside.size() - 1)) {
		...
		// CALCULATE THE HIGHEST EXISTING TIME IN THE TIME WINDOW 
		findingSuitableTimeIndex = true;
		while (findingSuitableTimeIndex && (outTrailHigh < outside.size() - 1)) {
		...
		*/
		...
		if (inValid && outValid) {
			...
			denominator = inside[inTrailHigh]->secs() - inside[inTrailLow]->secs();
			...
			if (result > limit) {
				...
				if (doorOpened == false) {
					// Record when the door opens.
					time = inside[inTrailLow]->getTimeOfDay();
					this->door.addTimeOfDay(time, true);
					// Needs different values for lastTime and time 
					// because time will be used as temporary holder of time
					// for both close and open, while lastTime just records
					// the last time the door opens.
					lastTime = time->getTotalTimeInSeconds();
				}
				doorOpened = true;
			}
				...
				if (doorOpened == true || !this->door.getHappened()) {
					...
					// Add the time to the total time open.
					if(this->door.getHappened())
						timeOpen += time->getTotalTimeInSeconds() - lastTime;
				}
				doorOpened = false;
			} // End checking for door open or close.
		} // End if valid in and out
	...
	// If door is still open, we add the remaining seconds to the total time.
	if(doorOpened == true) {
		timeOpen += 86400 - lastTime;
	}

	// Door cannot be opened for a negative amount of time.
	if (timeOpen < 0)
		timeOpen = 0;

	// Save.
	this->totalDoorOpenTime = timeOpen;
}