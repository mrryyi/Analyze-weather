...
void Manager::cutOffSlicesFromDays() {
	for (int i = 0; i < this->days.size(); i++) {
		this->days[i]->deleteData();
	}
}
...
void Manager::mergeSort(Day** arrr, int low, int high) {
	...
	if (low < high) {
		...
		// Merge left and right at this level.
...
void Manager::calculateDooring() {
	...
	timer.finish();
	std::cout << "Doorstuff calculated in " << timer.getSeconds() << " seconds;" << std::endl;
}

void Manager::printDoorStuff()
{
	for (int i = 0; i < days.size(); i++) {
		std::cout << days[i]->getDate()->getDateAsString() << ": ";
		if (days[i]->getDoor()->getHappened()) {
			std::cout << days[i]->getDoorOpenTime()->getStr();
		}
		else {
			std::cout << "Door was closed all day! Amazing.";
		}
		std::cout << std::endl;
...
void Manager::sortMenuAndDisplay() {
	...
	std::cout << "[8]  [both]    door open time per day" << std::endl;


	std::cout << "How do you want to sort? Input: ";
	
	int choice = getInteger(1, 8) - 1;
	...
	for (int i = 0; i < this->days.size(); i++) {
		...
		case DOOROPENSHENANIGANS:
			if (dayArray[i]->hasInsideData() && dayArray[i]->hasOutsideData()) {
				std::cout << "Door open total time: " <<dayArray[i]->getDoorOpenTime()->getStr();
			}
			else
				std::cout << "No inside or outside data for this day.";
			break;
...
void Manager::calculateSeasons() {
	...
	timer.finish();
	std::cout << "Seasons calculated in " << timer.getSeconds() << " seconds;" << std::endl;
}

void Manager::printSeasons() {
